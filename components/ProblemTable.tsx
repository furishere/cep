...
const STORAGE_KEY = "problem-table-state";
...
if (savedState) {
  try {
    const parsedState = JSON.parse(
      savedState
    ) as Record<number, ProblemState>;

    // Merge saved data with current problems
    setState((prev) => ({
      ...prev,
      ...parsedState,
    }));
  } catch (error) {
    console.error("Failed to load saved problem data:", error);
  }
}
...
// Save data only after localStorage has been loaded
useEffect(() => {
if (!hasLoaded) return;
...
return state[p.n]?.bookmarked;
}


    if (filter === "notes") {
      return state[p.n]?.notes.trim() !== "";
    }
...
return ( <div> <div className="mb-2 flex flex-wrap items-center justify-between gap-4"> <span className="font-mono text-[13px] text-gray-1"> <span>{solvedCount}</span> of {problems.length} solved </span> </div>
          ...
          <Fragment key={p.n}>
            ...
            {notesOpen && (
              <tr>
                <td
                  colSpan={5}
                  className="border-b border-line bg-line/20 px-1 py-3"
                >
                  <textarea
                    value={s?.notes ?? ""}
                    onChange={(e) =>
                      updateNotes(p.n, e.target.value)
                    }
